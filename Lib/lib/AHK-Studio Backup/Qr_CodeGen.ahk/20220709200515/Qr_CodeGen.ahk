...
SaveQRFile(Data,Path){	
	Global 	Pixlesize, Test, MATRIX_TO_PRINT, FILE_PATH_AND_NAME
	;if Sk_DevMode
		MsgBox,262208,SideKick ~ Dev Info %A_LineNumber%,QRData: %Data%`n`nFile Path: %Path%
	Pixlesize := 15
	test:= Data
...
{
  Msgbox, 0x10, Error, The QR data is blank is blank.
  Exit
}
