...
SaveQRFile(Data,Path){	
	...
	ifNotExist, %PathDIR%
		Msgbox,,,%PathDIR%`n`nDirectory Unavailable
	;if Sk_DevMode
	MsgBox,262208,SideKick ~ Dev Info QR_CodeGen %A_LineNumber%,QRData: `n%Data%`n`nFile Path: `n%Path%
...
MsgBox % Gdip_SaveBitmapToFile(pBitmap, FILE_PATH_AND_NAME)
Gdip_DisposeImage(pBitmap)
Gdip_DeleteGraphics(G)
Gdip_Shutdown(pToken)

Return
;MsgBox,0,Success,Created QR Code saved in clipboard and in file`n%FILE_PATH_AND_NAME%
;Goto START
;ExitApp
