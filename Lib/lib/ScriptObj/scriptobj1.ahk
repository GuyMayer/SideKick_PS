...
class script
{
	...
	*/
	...
	{ 
		...
		*/
		SetWorkingDir, C:\Stash\SideKick
		WGET1= ; %A_WorkingDir%\wget.exe ; from https://www.gnu.org/software/wget/
		ifnotexist,%wget1%
		{	msgbox,,,Downloading Wget to %A_WorkingDir%
			if A_Is64bitOS
				x1=https://eternallybored.org/misc/wget/current/wget64.exe
			
			else
				x1=https://eternallybored.org/misc/wget/current/wget.exe
			Msgbox,,, dowloading %x1%
			urldownloadtofile,%x1%,wget.exe
			sleep, 5000
		}
		
		;x:="http://cdn.clashofclans-tools.com/RLayout-301067-218.jpg"
		;ref=http://cdn.clashofclans-tools.com
		
		
		;run,%comspec% /k wget --no-parent --referer=%ref% "%x%"
		;MsgBox,,,% ref "`n`n" x 
		x := "https://zoom-photo.co.uk/sidekick" . "/version.txt" ; file
		ref=https://zoom-photo.co.uk/sidekick ; url
		Version1 = %A_WorkingDir%\version.txt
		VersionFile = %A_WorkingDir%\Update\version.txt
		
		
		MsgBox % A_IsAdmin "`n`n" A_PtrSize
		
		MsgBox,,,% ref "`n`n" x "`n`n" Version1 "`n`n" VersionFile
		FileDelete, %Version1%
		run, %comspec% /k wget --no-parent --referer=%ref% "%x%"
		...
		ifnotexist,%Version1%
		{	Progress, OFF
			msgbox,262160,,Error,Version file download Failure`n`n%Version1%
			SetWorkingDir := WkD
			return
		}
		else
		{	SetWorkingDir, C:\Stash\SideKick
			FileCreateDir,%A_WorkingDir%\Update
			FileMove, %Version1%, %VersionFile%,1
			FileDelete, %Version1%
			FileRead, FileVersion,%VersionFile%
		}
		msgbox,,msg,version %FileVersion%
		if (!(FileVersion) or instr(FileVersion, "Forbidden"))
		{
			Progress, OFF
			throw {code: ERR_NORESPONSE, msg: "File: " . vfile . "`n`n" . FileVersion .  "`n`n" . "There was an error trying to download the ZIP file.`n" . "The server did not respond."}
		}
		
		regexmatch(script.version, "\d+\.\d+\.\d+", loVersion)
		regexmatch(FileVersion, "\d+\.\d+\.\d+", remVersion)
		...
		if (!newversion)
			throw {code: ERR_CURRENTVER, msg: "You are using the latest version" . remVersion}
		else
		{
	...
	*/
	...
	autostart1(ByRef status, Toggle:=false)
	{
		msgbox % "Checking Reg"
		RegRead,KE, hkcu\software\microsoft\windows\currentversion\run, %a_scriptname%
		if !ErrorLevel
		{
			Msgbox,,,Found Key %KE%
			Status := True
	...
	*/
	about1(scriptName:="", version:="", author:="", homepagetext:="", homepagelink:="", donateLink:="", email:="")
	{
		static doc
